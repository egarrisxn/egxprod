...
import {addTimerSession, completeTimerSession} from '@/app/actions/timer'
...
export function SessionTimer() {
  ...
  const handleSessionEnd = useCallback(async () => {
    setIsRunning(false)
    if (sessionId) await completeTimerSession(sessionId)
    if (mode === 'work') {
  ...
  const startSession = async () => {
    setIsRunning(true)
    const session = await addTimerSession(mode, timeLeft)
