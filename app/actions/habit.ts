...
//! Log a habit day
export async function logHabitDay(id: number, date: string): Promise<string[] | null> {
  const supabase = await createClient()
  const {data: habit, error: fetchError} = await supabase
    .from('habits')
    .select('completed')
...
//! Unlog a habit day
export async function unlogHabitDay(id: number, date: string): Promise<string[] | null> {
  const supabase = await createClient()
  const {data: habit, error: fetchError} = await supabase
    .from('habits')
    .select('completed')
